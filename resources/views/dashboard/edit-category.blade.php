			...
			<form method="POST" action="{{ route('dashboard.categories.update', [$category->id]) }}" novalidate>
					...
					<div class="row mb-2">
							...
							<div class="col-md-12 @error('name') has-error @enderror">
									<input id="name" type="text" placeholder="Category name" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $category->name) }}" autocomplete="name" autofocus>
