            ...
            <div class="col-lg-8 col-md-10 mx-auto">
                ...
                @if (isset($search_query))
                    <p class="mt-0 text-muted">
                        ...
                        {{ $article_count > 1 ? 'articles' : 'article' }} containing <span
                            class="quote-inline">{{ $search_query }}</span>:
                    </p>
                @endif
                ...
                @if ($articles->hasPages())
                    <div class="clearfix">
                        <ul class="pagination">
                            <li class="next">
                                <a class="btn btn-primary {{ $articles->onFirstPage() ? 'disabled' : '' }}"
                                    href="{{ $articles->withQueryString()->previousPageUrl() }}">&larr; Newer Posts</a>
                            </li>
                            <li class="prev">
                                <a class="btn btn-primary {{ $articles->onLastPage() ? 'disabled' : '' }}"
                                    href="{{ $articles->withQueryString()->nextPageUrl() }}">Older Posts &rarr;</a>
                            </li>
                        </ul>
                    </div>
                @endif
