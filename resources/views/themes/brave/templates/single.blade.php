                ...
                <main class="content" id="article-content">
                    ...
                    <div class="row article-meta">
                        ...
                        <div class="right-half col-sm-12 col-sm-5 col-lg-4">
                            @php
                                $total_comments = $comments->sum(function ($comment) {
                                    return 1 + ($comment->replies->where('approved', 1)->count() ?? 0);
                                });
                            @endphp
                            <a class="comments" id="comments_status" href="#comments_container"
                                title="{{ $total_comments }} comments"><i class="fa fa-comments mr-1"></i>
                                {{ $total_comments ? $total_comments : 'No' }} comments</a> <strong>·</strong>

                            <a href="#" id="print_btn"
                                onclick="printJS({ printable: 'article-content', type: 'html', css: ['{{ asset('themes/' . $theme_directory . '/css/style.css') }}', '{{ asset('themes/' . $theme_directory . '/css/print.css') }}'] })"><i
